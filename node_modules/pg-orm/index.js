...
PgORM.prototype.fetchAll = function( SQLParams, callback ){
    ...
    if( table ){
        ...
        this.client.query( sql , function (err, result ) {
            ...
            if( result.rows.length >0 )callback( resultItems );
                else callback( null );
            return true;
        });
    }
...
PgORM.prototype.uploadFiles = function ( files, callBack ){
...
PgORM.prototype.checkBeforeSave = function ( callBack, files ){
    // checkbox boolean
    var thisObj = this;
    ...
    if( typeof( files ) == 'object'){
        // Закачиваем картинки
        thisObj.uploadFiles( files, callBack );
    }
        else {
        thisObj.saveModel( callBack );
    }
...
PgORM.prototype.save = function ( callback, files ){
    // Обработка необходимых полей перед сохранением
    this.checkBeforeSave( callback, files );
}

PgORM.prototype.saveModel = function ( callback ){
    var queue = require("queue");
    var q = queue();
    ...
    if( this.getAttribute("id") > 0 ){
        if( typeof( attributeList ) == 'object'){
                    ...
                    q.push( function(next){
                        thisObj.getAttribute(field, function( value ){
                            ...
                            if( value && typeof( value ) == "object"){
                                if( sql )sql += ', ';
                                sql += field+"='" + value.getAttribute("id") + "'";
                            }
            ...
            // Когда все задачи закончатся запустится callBack
            q.start(function(err) {
                if( sql && thisObj.validate() ){
                    sql = 'UPDATE '+thisObj.tableName+' SET ' + sql + ' WHERE id='+thisObj.getAttribute("id")+'::int';
                    thisObj.sql( sql, callback );
                }
                else {
...
PgORM.prototype.saveRelationsHasMany = function( callBack ){
    var hasRelation = false;
    for( relation in this.relations ){
        if( this.relations[relation][0] == "HAS_MANY" ){
            hasRelation = true;
            /*
            Нужно вытащить все значения переденные через POST в этом поле и сздать записи в таблице cat_relation
             */
        }
    }

    if( !hasRelation )callBack();
}
...
PgORM.prototype.getForm = function( callBack ){
    ...
    if( this.attributesRule )
    {
        ...
        saveFields.forEach( function( field ){
            ...
            q.push( function( next ){
                thisObj.getInput(field, thisObj.getAttribute(field) ,typeFields, "field_" +field, field, placeholder, required,
                    function ( input ){
...
PgORM.prototype.getInput = function ( field, value, typeFields, id, name, placeholder, required, callBack ){
    var input;
    var type;
    var inputValue;
            ...
            if( listValueModel != undefined && typeof( listValueModel ) == 'object' ){
                listValueModel.fetchAll({order:"name"}, function ( items ){
                    if( type == "relations-BELONGS_TO" ){
                        ...
                        for( var i=0;i<items.length;i++ ){
                            var id = items[i].getAttribute("id");
                            input += '<div class="checkbox"><label><input class="flat" name="' + name + '[]" type="checkbox" value="'+id+'"> '+items[i].getAttribute("name")+'</label></div>';
                        }
